...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = `
      query TokenInfo($addresses: [Address!]!, $networks: [Network!]) {
    ...
    // In production, you would parse the actual Zapper response
    const marketData: TokenMarketData = {
      tokenAddress,
      price: Math.random() * 10, // Mock price
      priceChange24h: (Math.random() - 0.5) * 20, // Mock price change between -10% and +10%
